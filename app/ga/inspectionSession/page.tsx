...
function page() {
  ...
  const sessionKey = section ? `gaInspectionSession_${section}` : "";
  const dateKey = section ? `gaInspectionSessionDate_${section}` : "";
  ...
  useEffect(() => {
    if (!section) {
      router.replace("/ga/dashboard/inspections");
      return;
    }
    const existing = localStorage.getItem(sessionKey);
    const existingDate = localStorage.getItem(dateKey);
    ...
    if (existing && existingDate) {
      ...
      localStorage.setItem(dateKey, newDate.toISOString());
      setTheSessionId(newId);
      setSessionDate(newDate.toISOString());
    }
  }, [section, sessionKey, dateKey, router]);
  ...
  useEffect(() => {
    if (!theSessionId || !section) return;
    let isMounted = true;

    async function fetchResidents() {
      try {
        const response = await apiFetch("api/ga/inspections/session", {
          method: "POST",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ sessionId: theSessionId, section }),
        });
        const result: {
          msg: string;
        ...
        if (result.roomsChecked.length < 1) {
          ...
          setSessionDate(localStorage.getItem(dateKey));
        } else {
          ...
          setTheSessionId(localStorage.getItem(sessionKey));
          setSessionDate(localStorage.getItem(dateKey));
        }
  ...
  }, [theSessionId, section, sessionKey, dateKey, router]);
