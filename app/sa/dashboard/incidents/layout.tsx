...
import { SAWorkspaceContext } from "@/context/SAWorkspaceContext";
...
export default function SAIncidentsLayout({ children }: { children: ReactNode }) {
  ...
  const load = useCallback(async () => {
    const res = await apiFetch("api/sa/workspace", { method: "GET" });
    if (res.status === 401) {
    ...
    <SAWorkspaceContext.Provider
      value={{
        ...
        refreshIncidents: async () => {
          const res = await apiFetch("api/sa/incidents", { method: "GET" });
          const data = await res.json();
          if (res.ok) {
            setWorkspace((w) =>
