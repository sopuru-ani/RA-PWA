...
export default function SAResidentDetailPage() {
  ...
  useEffect(() => {
    async function load() {
      const res = await apiFetch(`api/sa/residents/${id}`, { method: "GET" });
    ...
    <div className="flex flex-col space-y-4 pb-4 mx-3 h-full">
      ...
      <Card>
        <CardContent className="pt-4 space-y-2 text-sm">
          ...
          <p>
            <span className="font-medium">Student ID:</span> {resident.studentId}
          </p>
          {resident.raEmail ? (
            <p>
          ...
          {resident.gaEmail ? (
            <p>
              <span className="font-medium">{roleLabelShort("GA")}:</span>{" "}
              {resident.gaEmail}
            </p>
          ) : null}
        </CardContent>
